...
func (v ZoneEnrollResponse) EnrollResponseCodeString() string {
	switch v.EnrollResponseCode {
	case 0x0000:
		return "Success"
	case 0x0001:
		return "Not supported"
	case 0x0002:
		return "No enroll permit"
	case 0x0003:
		return "Too many zones"
	}
	return zcl.Sprintf("%v", zcl.Zenum16(v.EnrollResponseCode))
}
func (v ZoneEnrollResponse) ZoneIdString() string {
	return zcl.Sprintf("%v", zcl.Zu8(v.ZoneId))
}

func (v ZoneEnrollResponse) String() string {
	var str []string
	str = append(str, "EnrollResponseCode["+v.EnrollResponseCodeString()+"]")
	str = append(str, "ZoneId["+v.ZoneIdString()+"]")
	return "ZoneEnrollResponse{" + zcl.StrJoin(str, " ") + "}"
}
...
func (v InitiateNormalOperationMode) String() string {
	var str []string
	return "InitiateNormalOperationMode{" + zcl.StrJoin(str, " ") + "}"
}
...
func (v ZoneStatusChangeNotification) ZoneStatusString() string {
	var bstr []string
	if zcl.BitmapTest([]byte(v.ZoneStatus), 0) {
		bstr = append(bstr, "Alarm 1")
	}
	if zcl.BitmapTest([]byte(v.ZoneStatus), 1) {
		bstr = append(bstr, "Alarm 2")
	}
	if zcl.BitmapTest([]byte(v.ZoneStatus), 2) {
		bstr = append(bstr, "Tamper")
	}
	if zcl.BitmapTest([]byte(v.ZoneStatus), 3) {
		bstr = append(bstr, "Battery")
	}
	if zcl.BitmapTest([]byte(v.ZoneStatus), 4) {
		bstr = append(bstr, "Supervision reports")
	}
	if zcl.BitmapTest([]byte(v.ZoneStatus), 5) {
		bstr = append(bstr, "Restore reports")
	}
	if zcl.BitmapTest([]byte(v.ZoneStatus), 6) {
		bstr = append(bstr, "Trouble")
	}
	if zcl.BitmapTest([]byte(v.ZoneStatus), 7) {
		bstr = append(bstr, "AC (mains)")
	}
	if zcl.BitmapTest([]byte(v.ZoneStatus), 8) {
		bstr = append(bstr, "Test")
	}
	if zcl.BitmapTest([]byte(v.ZoneStatus), 9) {
		bstr = append(bstr, "Battery defect")
	}
	return zcl.StrJoin(bstr, ", ")
}
func (v ZoneStatusChangeNotification) ExtendedStatusString() string {
	return zcl.Sprintf("%v", zcl.Zu8(v.ExtendedStatus))
}
func (v ZoneStatusChangeNotification) ZoneIdString() string {
	return zcl.Sprintf("%v", zcl.Zu8(v.ZoneId))
}
func (v ZoneStatusChangeNotification) DelayString() string {
	return zcl.Sprintf("%v", zcl.Zu16(v.Delay))
}

func (v ZoneStatusChangeNotification) String() string {
	var str []string
	str = append(str, "ZoneStatus["+v.ZoneStatusString()+"]")
	str = append(str, "ExtendedStatus["+v.ExtendedStatusString()+"]")
	str = append(str, "ZoneId["+v.ZoneIdString()+"]")
	str = append(str, "Delay["+v.DelayString()+"]")
	return "ZoneStatusChangeNotification{" + zcl.StrJoin(str, " ") + "}"
}

func (ZoneStatusChangeNotification) Name() string { return "Zone Status Change Notification" }
...
func (v ZoneEnrollRequest) ZoneStatusString() string {
	switch v.ZoneStatus {
	case 0x0000:
		return "Standard CIE"
	case 0x000D:
	...
	return zcl.Sprintf("%v", zcl.Zenum16(v.ZoneStatus))
}
func (v ZoneEnrollRequest) ManufacturerCodeString() string {
	return zcl.Sprintf("%v", zcl.Zu16(v.ManufacturerCode))
}

func (v ZoneEnrollRequest) String() string {
	var str []string
	str = append(str, "ZoneStatus["+v.ZoneStatusString()+"]")
	str = append(str, "ManufacturerCode["+v.ManufacturerCodeString()+"]")
	return "ZoneEnrollRequest{" + zcl.StrJoin(str, " ") + "}"
}
